...
/* xx_zstd_extractor.c - search raw data for ZSTD.
 *
 * Scans for:
 *   28 B5 2F FD at +0  ("(...")
 ...
 * Each candidate must be accepted by the zstd reader, which also measures it,
...
static const xx_file_type_t k_types[] = { XX_FILE_TYPE_ZSTD };

static Abstractformat *xx_zstd_search_open(xx_io_device *window) {
    xx_zstd *reader = xx_zstd_create(window, 0);
    return reader ? &reader->format : NULL;
}

static void xx_zstd_search_close(Abstractformat *format) {
    /* The format is the first member, so this is the reader itself. */
    xx_zstd_free((xx_zstd *)format);
}

static const xx_format_search_desc k_desc = {
    k_types, sizeof(k_types) / sizeof(k_types[0]),
    k_anchors, sizeof(k_anchors) / sizeof(k_anchors[0]),
    xx_zstd_search_open, xx_zstd_search_close
};

static xx_format_search_state *xx_zstd_create_format_search(
...
static const xx_format_search_info *xx_zstd_get_current_format_info(
    xx_format_extractor *self, xx_format_search_state *state) {
    (void)self;
    return xx_format_search_current(state);
}

static bool xx_zstd_format_search_find_next(xx_format_extractor *self,
...
static void xx_zstd_free_format_search(xx_format_extractor *self,
                                     xx_format_search_state *state) {
    (void)self;
    xx_format_search_free(state);
}

xx_format_extractor xx_zstd_extractor = {
    xx_zstd_create_format_search,
    xx_zstd_get_current_format_info,
    xx_zstd_format_search_find_next,
    xx_zstd_free_format_search
};
