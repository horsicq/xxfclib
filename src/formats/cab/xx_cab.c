...
bool xx_cab_unpack_current_archive_record(Abstractformat*f,xx_archive_record_state*s,xx_pd_struct*p){cab_context*c;cab_file*x;const xx_var*o;uint8_t*folder=NULL;size_t folder_size=0;const char*base=NULL;char*owned=NULL,*path=NULL;bool ok=false,created=false;if(!f||!s||s->format!=f||!s->has_record||!(c=(cab_context*)s->internal_state)||c->index>=c->file_count||(p&&xx_pd_is_stopped(p)))return false;x=&c->files[c->index];if(!cab_safe(x->name)||!cab_decode_folder(f,c,x->folder,&folder,&folder_size)||x->folder_offset>folder_size||x->size>folder_size-x->folder_offset)goto done;o=cab_opt(&s->options,XX_META_ID_OPT_UNPACK_PATH);if(!o){ok=true;goto done;}if(o->type==XX_VAR_TYPE_STRING||o->type==XX_VAR_TYPE_STRING_VIEW)base=xx_var_get_str(o);else if(o->type==XX_VAR_TYPE_WSTRING||o->type==XX_VAR_TYPE_WSTRING_VIEW){owned=xx_str_unicode_to_utf8(xx_var_get_wstr(o));base=owned;}if(!base)goto done;path=(base[0]&&base[xx_str_len(base)-1]!='/'&&base[xx_str_len(base)-1]!='\\')?xx_str_concat3(base,"/",x->name):xx_str_concat(base,x->name);if(!path||!xx_store_create_dirs_a(path,false))goto done;{xx_io_device*d=xx_io_file_open(path,"wb");size_t at=0;if(!d)goto done;created=true;ok=true;while(at<x->size){ssize_t n=xx_io_write(d,folder+x->folder_offset+at,x->size-at);if(n<=0||(size_t)n>x->size-at){ok=false;break;}at+=(size_t)n;}xx_io_close(d);}done:if(!ok&&created)xx_rt_remove(path);if(folder)xx_mem_free(folder);if(path)xx_str_free(path);if(owned)xx_str_free(owned);return ok;}
