...
#define XX_LZ4_MAX_STREAM_SIZE ((uint64_t)1024U * 1024U * 1024U)

#define XX_LZ4_XXH_PRIME1 UINT32_C(2654435761)
#define XX_LZ4_XXH_PRIME2 UINT32_C(2246822519)
#define XX_LZ4_XXH_PRIME3 UINT32_C(3266489917)
#define XX_LZ4_XXH_PRIME4 UINT32_C(668265263)
#define XX_LZ4_XXH_PRIME5 UINT32_C(374761393)
...
/* A small local xxHash32 implementation used solely to authenticate frame
 * descriptors.  It is independently written from the public LZ4 format. */
static uint32_t xx_lz4_xxh32(const uint8_t *data, size_t size) {
    ...
    while (offset < size) {
        hash += (uint32_t)data[offset++] * XX_LZ4_XXH_PRIME5;
        hash = xx_lz4_rotl32(hash, 11U) * XX_LZ4_XXH_PRIME1;
    }
    hash ^= hash >> 15U;
    hash *= XX_LZ4_XXH_PRIME2;
    hash ^= hash >> 13U;
    hash *= XX_LZ4_XXH_PRIME3;
    hash ^= hash >> 16U;
    return hash;
}
...
static bool xx_lz4_block_limit(uint8_t descriptor, size_t *limit) {
    ...
    switch ((descriptor >> 4U) & 7U) {
        case 4U: *limit = 64U * 1024U; return true;
        case 5U: *limit = 256U * 1024U; return true;
        case 6U: *limit = 1024U * 1024U; return true;
        case 7U: *limit = 4U * 1024U * 1024U; return true;
        default: return false;
    }
}

static bool xx_lz4_read_exact_at(xx_io_device *device, int64_t offset,
...
static bool xx_lz4_take(const uint8_t **cursor, const uint8_t *end,
                        size_t count) {
...
static bool xx_lz4_scan_frames(const uint8_t *source, size_t size,
    ...
    uint64_t total = 0U;
    uint64_t frames = 0U;
    /* Every block expands to at most the frame's declared blockMaxSize, so
     * blocks x block_limit is an exact ceiling on the output. That is what a
    ...
    while (cursor < end) {
        ...
        if (magic != XX_LZ4_FRAME_MAGIC || (size_t)(end - cursor) < 3U) {
            return false;
        }
        descriptor_start = cursor;
        flags = *cursor++;
        block_descriptor = *cursor++;
        if ((flags >> 6U) != 1U || (flags & UINT8_C(0x02)) != 0U ||
            (flags & UINT8_C(0x01)) != 0U ||
            (block_descriptor & UINT8_C(0x8F)) != 0U ||
            !xx_lz4_block_limit(block_descriptor, &block_limit)) {
            return false;
        }
        /* The content-size field is OPTIONAL: bit 0x08 says whether it is
         * present, and the lz4 CLI does not write it by default. Requiring it
         * -- and then reading eight bytes that are not there -- rejected every
         * ordinary .lz4 file. The codec in src/algo/lz4 already treats it as
         * optional; only this scanner did not. */
        content_size = 0U;
        ...
        ++cursor;
        if (content_size > XX_LZ4_MAX_STREAM_SIZE ||
            total > XX_LZ4_MAX_STREAM_SIZE - content_size) {
            return false;
        }
        total += content_size;
        ...
        for (;;) {
            ...
            if (block_size == 0U || block_size > block_limit ||
                !xx_lz4_take(&cursor, end, block_size)) {
                return false;
            }
            if (ceiling > XX_LZ4_MAX_STREAM_SIZE - (uint64_t)block_limit) {
                return false;
            }
            ceiling += (uint64_t)block_limit;
            if (block_checksum && !xx_lz4_take(&cursor, end, 4U)) {
                return false;
            }
        }
        if (content_checksum && !xx_lz4_take(&cursor, end, 4U)) return false;
...
static bool xx_lz4_scan_device(Abstractformat *self,
                               uint64_t *uncompressed_size,
                               uint64_t *frame_count,
                               int64_t *stream_size,
                               xx_pd_struct *pd) {
    ...
    if (total_size < self->base_address ||
        ...
        (uint64_t)(total_size - self->base_address) > XX_LZ4_MAX_STREAM_SIZE ||
...
static bool xx_lz4_write_all(xx_io_device *device, const void *data,
...
static bool xx_lz4_decode_stream(Abstractformat *self,
    ...
    if (declared_size != 0U) {
        ...
        if (ceiling > (uint64_t)SIZE_MAX) goto cleanup;
        output = (uint8_t *)xx_mem_alloc(ceiling == 0U ? 1U : (size_t)ceiling);
        if (!output || !xx_lz4_decompress_frames(input, (size_t)input_size,
                                                 output, (size_t)ceiling,
                                                 &written)) {
            goto cleanup;
        }
        declared_size = (uint64_t)written;
    }
    if (destination && !xx_lz4_write_all(destination, output, written, pd)) {
        goto cleanup;
    }
...
static bool xx_lz4_copy_options(xx_list_s *destination,
...
static const xx_var *xx_lz4_find_option(const xx_list_s *options,
                                        uint32_t meta_id) {
    size_t index;
...
static bool xx_lz4_populate_record(Abstractformat *self,
                                   xx_archive_record *record) {
    const xx_lz4 *archive;
    if (!self || !record || !self->base_info_handled || !self->is_valid ||
        self->format_size < 4) {
        return false;
    }
    archive = (const xx_lz4 *)self;
...
void xx_lz4_init(xx_lz4 *archive, xx_io_device *device,
                 int64_t base_address) {
    if (!archive) return;
    xx_mem_zero(archive, sizeof(*archive));
    xx_format_init(&archive->format, device, base_address);
    archive->format.endian = XX_ENDIAN_LITTLE;
    archive->format.file_type = XX_FILE_TYPE_LZ4;
    archive->format.format_type = XX_TYPE_ARCHIVE;
    archive->format.is_archive = true;
    xx_format_set_mime_type(&archive->format, "application/x-lz4");
    xx_format_set_extension(&archive->format, "lz4");
    archive->format.check_is_valid = xx_lz4_check_is_valid;
    archive->format.handle_base_info = xx_lz4_handle_base_info;
    archive->format.get_format_size = xx_lz4_get_format_size;
    archive->format.get_number_of_archive_records =
        xx_lz4_get_number_of_archive_records;
    archive->format.create_archive_records_reading =
        xx_lz4_create_archive_records_reading;
    archive->format.get_current_archive_record =
        xx_lz4_get_current_archive_record;
    archive->format.unpack_current_archive_record =
        xx_lz4_unpack_current_archive_record;
    archive->format.archive_record_move_to_next =
        xx_lz4_archive_record_move_to_next;
    archive->format.free_archive_records_reading =
        xx_lz4_free_archive_records_reading;
    archive->format.destroy = xx_lz4_vtable_destroy;
    archive->stream_end = -1;
}

xx_lz4 *xx_lz4_create(xx_io_device *device, int64_t base_address) {
    xx_lz4 *archive = (xx_lz4 *)xx_mem_alloc(sizeof(*archive));
    if (archive) xx_lz4_init(archive, device, base_address);
    return archive;
}

void xx_lz4_destroy(xx_lz4 *archive) {
    if (!archive) return;
...
static void xx_lz4_vtable_destroy(Abstractformat *self) {
    xx_lz4_destroy((xx_lz4 *)self);
}

void xx_lz4_free(xx_lz4 *archive) {
    if (!archive) return;
    xx_lz4_destroy(archive);
    xx_mem_free(archive);
}

bool xx_lz4_check_is_valid(Abstractformat *self, xx_pd_struct *pd) {
    uint64_t uncompressed_size;
    uint64_t frame_count;
    int64_t stream_size;
    return xx_lz4_scan_device(self, &uncompressed_size, &frame_count,
                              &stream_size, pd);
}

bool xx_lz4_handle_base_info(Abstractformat *self, xx_pd_struct *pd) {
    uint64_t uncompressed_size;
    uint64_t frame_count;
    int64_t stream_size;
    int64_t total_size;
    ...
    if (!self || !xx_lz4_check_is_valid(self, pd) ||
        !xx_lz4_decode_stream(self, NULL, &uncompressed_size, &frame_count,
                              &stream_size, pd)) {
        if (self) {
            archive = (xx_lz4 *)self;
            archive->number_of_frames = 0U;
    ...
    self->overlay_size = archive->stream_end < total_size
                             ...
                             : 0;
    self->file_type = XX_FILE_TYPE_LZ4;
...
int64_t xx_lz4_get_format_size(Abstractformat *self, xx_pd_struct *pd) {
...
uint64_t xx_lz4_get_number_of_archive_records(Abstractformat *self,
                                               xx_pd_struct *pd) {
    if (!self || (!self->base_info_handled &&
                  !xx_format_handle_base_info(self, pd))) {
        return 0U;
...
bool xx_lz4_unpack_to_device(xx_lz4 *archive, xx_io_device *destination,
                             xx_pd_struct *pd) {
    uint64_t uncompressed_size;
    uint64_t frame_count;
    int64_t stream_size;
    if (!archive || !destination ||
        ...
        !xx_lz4_decode_stream(&archive->format, destination,
...
xx_archive_record_state *xx_lz4_create_archive_records_reading(
    ...
    if (!xx_lz4_copy_options(&state->options, options) ||
        !xx_lz4_populate_record(self, &state->current_record)) {
...
const xx_archive_record *xx_lz4_get_current_archive_record(
...
bool xx_lz4_archive_record_move_to_next(Abstractformat *self,
                                        xx_archive_record_state *state,
                                        xx_pd_struct *pd) {
...
bool xx_lz4_unpack_current_archive_record(Abstractformat *self,
                                          xx_archive_record_state *state,
                                          xx_pd_struct *pd) {
    ...
    bool result;
    xx_lz4 *archive = (xx_lz4 *)self;

    if (!self || !state || state->format != self || !state->has_record ||
        (pd && xx_pd_is_stopped(pd))) {
        return false;
    }
    path_value = xx_lz4_find_option(&state->options,
                                    XX_META_ID_OPT_UNPACK_PATH);
    if (!path_value) {
        uint64_t size;
        uint64_t frames;
        int64_t compressed_size;
        return xx_lz4_decode_stream(self, NULL, &size, &frames,
                                    &compressed_size, pd) &&
    ...
    if (base_path[0] != '\0' &&
        ...
        base_path[xx_str_len(base_path) - 1U] != '\\') {
        destination_path = xx_str_concat3(base_path, "/", XX_LZ4_PAYLOAD_NAME);
    } else {
        destination_path = xx_str_concat(base_path, XX_LZ4_PAYLOAD_NAME);
    }
    if (owned_path) xx_str_free(owned_path);
    ...
    }
    {
        ...
        result = output && xx_lz4_unpack_to_device(archive, output, pd);
...
void xx_lz4_free_archive_records_reading(Abstractformat *self,
                                         xx_archive_record_state *state) {
    (void)self;
    xx_archive_record_state_free(state);
}

uint64_t xx_lz4_get_number_of_frames(const xx_lz4 *archive) {
    return archive ? archive->number_of_frames : 0U;
}

uint64_t xx_lz4_get_uncompressed_size(const xx_lz4 *archive) {
    return archive ? archive->uncompressed_size : 0U;
}

int64_t xx_lz4_get_stream_end(const xx_lz4 *archive) {
