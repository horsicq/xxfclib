 ...
 *
 *   table, 25 bytes per member, starting at 2:
 *     +0x00  4  u32 LE   payload size
 *     +0x04  4  u32 LE   DOS packed date/time
 *     +0x08  4  u32 LE   absolute payload offset
 *     +0x0c 13  char[13] name, NUL terminated inside the field; the bytes
 *                        after the NUL are stale and must be ignored
 *
 ...
 * must follow the previous one exactly and the last must end precisely on
 * end-of-file.  That is what stands in for a magic number, since the format
 * has none.
 *
 * All 5 corpus samples in F:\ARC\ARC\RSC parse, for 381 members total.
 */
...
#else
#define XX_RSC_FILE_TYPE XX_FILE_TYPE_UNKNOWN
#endif

#define XX_RSC_METHOD_STORE 0U
#define XX_RSC_HEADER_SIZE 2
#define XX_RSC_ENTRY_SIZE 25
#define XX_RSC_NAME_SIZE 13
/* The 16-bit count is its own ceiling; the table-fits-in-the-file check is
 * what actually bounds the allocation. */
#define XX_RSC_MAX_MEMBERS 0xffffU

typedef struct xx_rsc_member_s {
    char *name;
...
typedef struct xx_rsc_stream_s {
    xx_rsc_member *items;
    size_t count;
    size_t capacity;
    size_t index;
...
static uint16_t xx_rsc_be16(const uint8_t *data) {
    return (uint16_t)((uint16_t)data[1] | ((uint16_t)data[0] << 8));
}

static uint32_t xx_rsc_be32(const uint8_t *data) {
...
static xx_rsc_stream *xx_rsc_parse(Abstractformat *self,
    ...
    if (span < XX_RSC_HEADER_SIZE + 2 * XX_RSC_ENTRY_SIZE) return NULL;
    if (!xx_rsc_read_at(self, self->base_address, head, sizeof(head))) {
        return NULL;
    }
    ...
    if (table_size > span - XX_RSC_HEADER_SIZE) return NULL;
    ...
    stream = (xx_rsc_stream *)xx_mem_alloc(sizeof(*stream));
    if (!stream) goto fail;
    ...
    for (index = 0U; index < count; ++index) {
        ...
        /* The name is NUL terminated inside its 13-byte field; bytes after
         * the NUL are stale and must not be read as name. */
        while (name_size < XX_RSC_NAME_SIZE && raw[name_size] != 0x00U) {
            ++name_size;
        }
        ...
        member.method = XX_RSC_METHOD_STORE;
        if (!xx_rsc_add(stream, &member)) {
...
void xx_rsc_init(xx_rsc *archive, xx_io_device *device,
    ...
    archive->format.file_type = XX_RSC_FILE_TYPE;
    archive->format.format_type = XX_TYPE_ARCHIVE;
    archive->format.is_archive = true;
    xx_format_set_mime_type(&archive->format, "application/x-rsc-resource");
    xx_format_set_extension(&archive->format, "rsc");
...
xx_rsc *xx_rsc_create(xx_io_device *device, int64_t base_address) {
    ...
    xx_rsc_init(archive, device, base_address);
    return archive;
}
...
bool xx_rsc_unpack_current_archive_record(Abstractformat *self,
    ...
    if (!path_option) {
        ...
        result = xx_rsc_decode(self, member, &plain, &plain_size, pd);
        xx_mem_free(plain);
        return result;
    }
