...
/* xx_lz5_extractor.c - search raw data for LZ5.
 *
 * Scans for:
 *   05 22 4D 18 at +0  ("..M.")
 ...
 * Each candidate must be accepted by the lz5 reader, which also measures it,
...
static const uint8_t k_anchor0[] = { 0x05, 0x22, 0x4D, 0x18 };
...
static const xx_file_type_t k_types[] = { XX_FILE_TYPE_LZ5 };

static Abstractformat *xx_lz5_search_open(xx_io_device *window) {
    xx_lz5 *reader = xx_lz5_create(window, 0);
    return reader ? &reader->format : NULL;
}

static void xx_lz5_search_close(Abstractformat *format) {
    /* The format is the first member, so this is the reader itself. */
    xx_lz5_free((xx_lz5 *)format);
}

static const xx_format_search_desc k_desc = {
    k_types, sizeof(k_types) / sizeof(k_types[0]),
    k_anchors, sizeof(k_anchors) / sizeof(k_anchors[0]),
    xx_lz5_search_open, xx_lz5_search_close
};

static xx_format_search_state *xx_lz5_create_format_search(
...
static const xx_format_search_info *xx_lz5_get_current_format_info(
    xx_format_extractor *self, xx_format_search_state *state) {
    (void)self;
    return xx_format_search_current(state);
}

static bool xx_lz5_format_search_find_next(xx_format_extractor *self,
...
static void xx_lz5_free_format_search(xx_format_extractor *self,
                                     xx_format_search_state *state) {
    (void)self;
    xx_format_search_free(state);
}

xx_format_extractor xx_lz5_extractor = {
    xx_lz5_create_format_search,
    xx_lz5_get_current_format_info,
    xx_lz5_format_search_find_next,
    xx_lz5_free_format_search
};
