...
#define XX_LIZARD_MAX_STREAM_SIZE ((uint64_t)1024U * 1024U * 1024U)
...
static uint32_t xx_lizard_xxh32(const uint8_t *data, size_t size) {
    ...
    while (offset < size) {
        hash += (uint32_t)data[offset++] * XX_LIZARD_XXH_P5;
        hash = xx_lizard_rotl32(hash, 11U) * XX_LIZARD_XXH_P1;
    }
    hash ^= hash >> 15U;
...
static bool xx_lizard_scan_frames(const uint8_t *source, size_t size,
    ...
    while (cursor < end) {
        ...
        if (magic != XX_LIZARD_MAGIC || (size_t)(end - cursor) < 3U) {
            return false;
        }
...
void xx_lizard_init(xx_lizard *archive, xx_io_device *device,
    ...
    archive->format.file_type = XX_FILE_TYPE_LIZARD;
    archive->format.format_type = XX_TYPE_ARCHIVE;
    archive->format.is_archive = true;
    xx_format_set_mime_type(&archive->format, "application/x-lizard");
    xx_format_set_extension(&archive->format, "lizard");
...
bool xx_lizard_unpack_current_archive_record(Abstractformat *self,
    ...
    }
    {
        ...
        created = output != NULL;
        result = output && xx_lizard_unpack_to_device(archive, output, pd);
