...
/* xx_tar_lz4_extractor.c - search raw data for TAR.LZ4.
 *
 * Scans for:
 *   04 22 4D 18 at +0  ("..M.")
 ...
 * Each candidate must be accepted by the tar_lz4 reader, which also measures it,
...
static const uint8_t k_anchor0[] = { 0x04, 0x22, 0x4D, 0x18 };
...
static const xx_file_type_t k_types[] = { XX_FILE_TYPE_TAR_LZ4 };

static Abstractformat *xx_tar_lz4_search_open(xx_io_device *window) {
    xx_tar_lz4 *reader = xx_tar_lz4_create(window, 0);
    return reader ? &reader->format : NULL;
}

static void xx_tar_lz4_search_close(Abstractformat *format) {
    /* The format is the first member, so this is the reader itself. */
    xx_tar_lz4_free((xx_tar_lz4 *)format);
}

static const xx_format_search_desc k_desc = {
    k_types, sizeof(k_types) / sizeof(k_types[0]),
    k_anchors, sizeof(k_anchors) / sizeof(k_anchors[0]),
    xx_tar_lz4_search_open, xx_tar_lz4_search_close
};

static xx_format_search_state *xx_tar_lz4_create_format_search(
...
static const xx_format_search_info *xx_tar_lz4_get_current_format_info(
    xx_format_extractor *self, xx_format_search_state *state) {
    (void)self;
    return xx_format_search_current(state);
}

static bool xx_tar_lz4_format_search_find_next(xx_format_extractor *self,
...
static void xx_tar_lz4_free_format_search(xx_format_extractor *self,
                                     xx_format_search_state *state) {
    (void)self;
    xx_format_search_free(state);
}

xx_format_extractor xx_tar_lz4_extractor = {
    xx_tar_lz4_create_format_search,
    xx_tar_lz4_get_current_format_info,
    xx_tar_lz4_format_search_find_next,
    xx_tar_lz4_free_format_search
};
