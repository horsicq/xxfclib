 ...
 *
 * Native reader for the FDI floppy image.  The sector data is stored, so the
 * reader only has to validate the header and publish the data area.
 */
...
#else
#define XX_FDI_FILE_TYPE XX_FILE_TYPE_UNKNOWN
#endif

#define FDI_MAX_MEMBERS 65536U
#define FDI_MAX_OUTPUT (64U * 1024U * 1024U)

typedef struct fdi_member_s {
    char *name;
...
static uint32_t fdi_le32(const uint8_t *b) {
...
void xx_fdi_init(xx_fdi *archive, xx_io_device *device, int64_t base_address) {
    ...
    archive->format.file_type = XX_FDI_FILE_TYPE;
    archive->format.format_type = XX_TYPE_ARCHIVE;
    archive->format.is_archive = true;
    xx_format_set_mime_type(&archive->format, "application/x-fdi-image");
...
bool xx_fdi_handle_base_info(Abstractformat *format, xx_pd_struct *pd) {
    ...
    xx_fdi *archive;
    (void)pd;
    if (!format || !fdi_parse(format, &stream)) return false;
    archive = (xx_fdi *)format;
...
xx_archive_record_state *xx_fdi_create_archive_records_reading(
    ...
    (void)pd;
    if (!fdi_parse(format, &stream)) return NULL;
...
bool xx_fdi_unpack_current_archive_record(Abstractformat *format,
    ...
    path_option = fdi_option(&state->options, XX_META_ID_OPT_UNPACK_PATH);
    if (!path_option) {
        result = true;
        goto done;
    }
