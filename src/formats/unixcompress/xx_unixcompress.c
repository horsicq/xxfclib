...
typedef struct xx_unixcompress_sink_s {
    xx_io_device *target;
    xx_pd_struct *pd;
    uint64_t total;
    uint64_t limit;
    bool failed;
} xx_unixcompress_sink;

static void xx_unixcompress_vtable_destroy(Abstractformat *self);

static ssize_t xx_unixcompress_sink_write(xx_io_device *device,
                                          const void *data, size_t size) {
    xx_unixcompress_sink *sink =
        device ? (xx_unixcompress_sink *)device->priv : NULL;
    size_t done = 0U;
...
/* Unix compress does not carry an end marker or a checksum.  A standalone
 * member therefore occupies the entire supplied device range. */
static bool xx_unixcompress_decode_stream(Abstractformat *self,
    ...
    if (total_size < self->base_address ||
        total_size - self->base_address < 3 ||
        (uint64_t)(total_size - self->base_address) > XX_UNIXCOMPRESS_MAX_INPUT) {
        return false;
    }
    input_size = total_size - self->base_address;
    output.target = destination;
    output.pd = pd;
    output.limit = XX_UNIXCOMPRESS_MAX_OUTPUT;
    sink.write = xx_unixcompress_sink_write;
    sink.priv = &output;
    if (!xx_compress_decode_device(self->device, self->base_address,
...
bool xx_unixcompress_check_is_valid(Abstractformat *self, xx_pd_struct *pd) {
    uint64_t size;
    int64_t stream_size;
    return xx_unixcompress_decode_stream(self, NULL, &size, &stream_size, pd);
}
...
bool xx_unixcompress_handle_base_info(Abstractformat *self, xx_pd_struct *pd) {
    ...
    int64_t stream_size;
    xx_unixcompress *archive;
    if (!self || !xx_unixcompress_decode_stream(self, NULL, &size,
                                                &stream_size, pd)) {
        if (self) {
...
bool xx_unixcompress_unpack_to_device(xx_unixcompress *archive,
    ...
    if (!archive || !destination ||
        ...
        !xx_unixcompress_decode_stream(&archive->format, destination, &size,
                                       &stream_size, pd)) {
        return false;
    }
...
bool xx_unixcompress_unpack_current_archive_record(
    ...
    if (!path_value) {
        uint64_t size;
        int64_t stream_size;
        return xx_unixcompress_decode_stream(self, NULL, &size, &stream_size,
                                             pd) &&
